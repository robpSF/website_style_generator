...
def main():
    ...
    if st.button("Generate JSON"):
        if company_name and company_bio and header_image_url:
            article_prompt = f"Write a website article based on the following company bio. Don't include a title and keep to less than 500 words:\n\n{company_bio}\n\nArticle:"
            article = generate_text(article_prompt)
            
            title_prompt = f"Generate a title for a website article based on the following company bio. Don't encapsulate in quotes:\n\n{company_bio}\n\nTitle:"
            title = generate_text(title_prompt, max_tokens=50).strip()
            
            subtitle_prompt = f"Generate a subtitle for a website article based on the following company bio. Don't encapsulate in quotes:\n\n{company_bio}\n\nSubtitle:"
